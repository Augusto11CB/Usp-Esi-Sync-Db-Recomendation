...
class AcessaBD():
    ...
    def ordene_menor_preco(self, lista):
        if not lista:
            return []
        self.cur.execute('select idproduto from produto where idproduto in %s order by preco;', (tuple(lista),))
        resp = self.cur.fetchall()
        return [i[0] for i in resp]

    def ordene_maior_preco(self, lista):
        if not lista:
            return []
        self.cur.execute('select idproduto from produto where idproduto in %s order by preco desc;', (tuple(lista),))
        resp = self.cur.fetchall()
        return [i[0] for i in resp]

    def ordene_popularidade(self, lista):
        if not lista:
            return []
        self.cur.execute('select idproduto from produto where idproduto in %s order by vizualizado desc;', (tuple(lista),))
        resp = self.cur.fetchall()
        return [i[0] for i in resp]

    def busca_sugestoes(self, caixa_texto):
        self.cur.execute('select busca, count(*) as t from buscas where busca like \'%'+caixa_texto+'%\' group by busca order by t desc limit 5;')
    ...
    # Buscas
    def devolve_produtos(self, max_preco, min_preco):
        self.cur.execute('select t.idproduto, t.arrayTAG from produto join tag as t on produto.idProduto = t.idproduto where produto.preco between \''+str(min_preco)+'\' and \''+str(max_preco)+'\';')
        resp = self.cur.fetchall()
    ...
    def devolve_produtos_categoria_busca(self, categoria, max_preco, min_preco):
        self.cur.execute('select t.idproduto, t.arrayTAG from produto join tag as t on produto.idProduto = t.idproduto and produto.categoria = \''+categoria+'\' and produto.preco between \''+str(min_preco)+'\' and \''+str(max_preco)+'\';')
        resp = self.cur.fetchall()
        return [i[0] for i in resp], [i[1] for i in resp]
    ...
    def devolveNrecomendadosParaCliente(self, id_cliente, n):
        self.cur.execute(
            ...
            'select DISTINCT visualizacao.idproduto from visualizacao join (select visualizacao.idcliente as cid from (select idproduto as ed from visualizacao where idcliente = \''+str(id_cliente)+'\') as produtos_vistos join visualizacao on ed = visualizacao.idProduto) as clientes_relacionados on clientes_relacionados.cid = visualizacao.idCliente where visualizacao.idProduto not in (select idproduto from visualizacao where idcliente =\''+str(id_cliente)+'\') limit \''+str(n)+'\';'
        )
            #
            #
        ...
        resposta = [f[0] for f in self.cur.fetchall()]
        if len(resposta) == n:
            return resposta

        self.cur.execute(
            'select idproduto from visualizacao group by idProduto order by count(*) desc limit \'' + str(n) + '\';')
        resp = self.cur.fetchall()
        return [i[0] for i in resp]